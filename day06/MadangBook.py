...
class qtApp(QMainWindow):
    ...
    def btnNewClicked(self): # 신규 버튼 클릭
        ...
        # 선택한 데이터에서 신규를 누르면 self.txtBookId 사용여부는 변경해줘야 한다
        self.txtBookId.setEnabled(True) # 사용


    def btnSaveClicked(self): # 저장 버튼 클릭
        # 입력검증(Validation Check) 반드시 해야함
        # 1. 텍스트 박스를 비워두고 저장 버튼을 누르면 안된다
        ...
        warningMsg = '' # 경고메시지
        isVaild = True # 빈 값이 있으면 False로 변경
        ...
        if isVaild == False: # 위 입력 값 중에 하나라도 빈 값이 존재
    ...
    def btnDelClicked(self): # 삭제 버튼 클릭
        ...
        # 삭제시는 삭제여부를 물어봐야함
        re = QMessageBox.question(self, '삭제경고', '정말로 삭제하시겠습니까?', QMessageBox.Yes | QMessageBox.No)
    ...
    def makeTable(self, data): # tblBooks 위젯을 데이터와 컬럼 생성해주는 함수
        self.tblBooks.setColumnCount(4) # booid, bookname, publisher, price
        ...
        self.tblBooks.setColumnWidth(3, 80) #  책 이름 컬럼 넓이
        # 컬럼 더블클릭 금지
