-- 555p 부터 시작
 ...
 /* 5. 현재의 날짜 타입을 날짜 함수를 통해서 확인
    2006년 5월 20일 부터 2007년 5월 20일 사이에 고용된 사원의 이름(First + Last), 사원번호
    고용일자를 입사일이 빠른 순으로 정렬하시오. (18개행) */
...
-- 서브쿼리
/* 사원의 급여 정보 중 최소 급여를 받는 사원의 이름을, 업무, 급여, 입사일 출력(21개행) */
...
-- CASE 연산자(프로그래밍적인)
/*  107명의 직원 중 HR_REP(10%), MK_REP(12%), PR_REP(15%), SA_REP(18%), IT_PROG(20%) 인상 */
SELECT EMPLOYEE_ID
     , FIRST_NAME + ' ' + LAST_NAME AS 'name'
     , JOB_ID
     , SALARY
...
-- 사원들의 부서별 급여 기준으로 내림차순으로 정렬, 순위를 표시하시오. (107개행)
  SELECT EMPLOYEE_ID
     , LAST_NAME
     , DEPARTMENT_ID
     , RANK() OVER (ORDER BY SALARY DESC) -- 동등순위 중복 증가 타입
     , DENSE_RANK() OVER (ORDER BY SALARY DESC) -- 동등순위 순차 증가 타입
  FROM employees
